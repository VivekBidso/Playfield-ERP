...
async def get_rm_inventory(
    ...
    for item in items:
        rm = await db.raw_materials.find_one({"rm_id": item["rm_id"]}, {"_id": 0, "name": 1, "description": 1, "category": 1, "default_unit": 1})
        if rm:
            # Description is now stored in DB - read directly
            item["rm_name"] = rm.get("description") or rm.get("name") or ""
