...
async def bulk_update_vendor_payment_terms(file: UploadFile = File(...)):
    """Bulk-UPDATE existing vendors' Payment Terms from an Excel file.

    Accepts the same Excel format as `/vendors/bulk-upload` (vendor template
    with a `Payment Terms` column added). Matches by `Name` (case-insensitive).
    Does NOT create new vendors — vendors not found in DB are reported under
    `not_found`. Other columns (GST, address, etc.) are ignored — only
    payment_terms is updated.

    Allowed Payment Terms values: Due on Receipt, Net 7, Net 15, Net 30,
    Net 45, Net 60 (or their underscore forms DUE_ON_RECEIPT, NET_7, ...).
    """
    from models.vendor import ALLOWED_PAYMENT_TERMS
    import openpyxl
    import io
    ...
    field_indices = {header_map[h]: i for i, h in enumerate(headers) if h in header_map}
    ...
    label_to_code = {
        "due on receipt": "DUE_ON_RECEIPT", "due_on_receipt": "DUE_ON_RECEIPT",
        "net 7": "NET_7", "net_7": "NET_7", "net7": "NET_7",
        "net 15": "NET_15", "net_15": "NET_15", "net15": "NET_15",
        "net 30": "NET_30", "net_30": "NET_30", "net30": "NET_30",
        "net 45": "NET_45", "net_45": "NET_45", "net45": "NET_45",
    ...
    for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        try:
            name_val = row[field_indices['name']]
            if not name_val:
                continue
            ...
            pt_raw = row[field_indices['payment_terms']]
            if pt_raw is None or str(pt_raw).strip() == "":
                errors.append(f"Row {idx} ({name}): Payment Terms is blank")
                continue
            pt_key = str(pt_raw).strip().lower()
            payment_terms = label_to_code.get(pt_key, str(pt_raw).strip().upper())
            if payment_terms not in ALLOWED_PAYMENT_TERMS:
